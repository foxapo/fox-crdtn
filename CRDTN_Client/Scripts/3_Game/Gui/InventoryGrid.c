modded class InventoryGrid extends ScriptedWidgetEventHandler
{
	override void GenerateQuickBarBackgroundTiles(int count)
	{
		...
		{
			Widget root_widget = GetGame().GetWorkspace().CreateWidgets("CRDTN_Client/Layouts/Inventory/inventoryGridBackground.layout", m_Root);
	...
	override void AddItem(InventoryItem item, vector data, vector rotation)
	{
		...
		Widget item_w_bck = GetGame().GetWorkspace().CreateWidgets("CRDTN_Client/Layouts/Inventory/inventoryGridItem.layout", bck);
